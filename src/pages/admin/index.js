...
export default function Adicionar() {
    ...
    async function salvarClick() {
        try{
            ...
            if(!imagem) throw new Error('Por favor envie uma imagem!');
            if(id === 0){
                
                // Validações                                
                if(parseFloat(preco) < 0) throw new Error('Digite um valor valido!');
                if(nome.length > 200 || descricao.length > 200) throw new Error('Numero de caracteres excedido!')
                if( !nome.trim()        ||
                    !preco              ||
                    !categoria.trim()   ||
                    !descricao.trim()   )
                ...
                // Setting a new id for the product
